...
* If you enable or disable the Catalog Permissions/Shared Catalog/B2B features, the `catalog_data_exporter_product_overrides` indexer is not updated and is incorrectly marked as `valid`. Use `bin/magento saas:resync --feed=productOverrides` to fix the issue.
