...
[!DNL Live Search] is a set of standalone [packages](#live-search-packages) that replaces standard Magento Open Source and Adobe Commerce search capabilities.

This topic provides instructions to do the following:

- [Installing [!DNL Live Search]](#before-you-begin) (Methods 1 and 2)
- [Updating [!DNL Live Search]](#updating-live-search)
- [Uninstalling [!DNL Live Search]](#uninstalling-live-search)

[!TIP] The installation takes place from the command line of the server. Rather than retyping each command, hover over the far right of the code box, click the **Copy** link, and paste it into the command line. If you do not have experience working from the command line, ask your system integrator or developer for assistance.
...
The [!DNL Live Search] category search / category API has the following supported limits and static boundaries and that are inherent to the design of the software:
...
In this scenario, storefront operations are interrupted while the [!DNL Live Search] service indexes all products in the catalog. During the installation, [!DNL Live Search] modules are enabled and Elasticsearch modules are disabled.

1. Install {{site.data.var.ee}} 2.4.x without [!DNL Live Search].
...
1. To download the `live-search` package, run the following from the command line:
   ...
   For more information, see the list of [!DNL Live Search] [dependencies](#live-search-dependencies) that are captured by Composer.

1. Run the following commands to disable Elasticsearch and related modules and install [!DNL Live Search]:
...
1. Run the following command to temporarily disable the [!DNL Live Search] modules that serve storefront search results.
   ...
   Elasticsearch continues to manage search requests from the storefront while the [!DNL Live Search] service synchronizes the catalog data and indexes products in the background.
...
1. Run the following commands to disable Elasticsearch modules, enable [!DNL Live Search] modules, and run setup:
...
To verify that the catalog data has been exported from your {{site.data.var.ee}} instance and is synchronized for [!DNL Live Search], look for entries in the following tables:
...
To uninstall [!DNL Live Search], go to [Uninstall modules]({{ site.devdocs_url }}/guides/v2.4/install-gde/install/cli/install-cli-uninstall-mods.html).
...
|`module-live-search-adapter`|Routes search requests from the storefront to the [!DNL Live Search] service and renders the results in the storefront. <br />- Category browse - Routes requests from the storefront [top navigation]({% link catalog/navigation-top.md %}) to the search service.<br />- Global search - Routes requests from the [quick search]({% link catalog/search-quick.md %}) box in the upper-right of the storefront to the [!DNL Live Search] service.|
...
The following [!DNL Live Search] dependencies are captured by Composer:
